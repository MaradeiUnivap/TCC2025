...
class MeuTokenJWT {
    ...
    gerarToken(parametroClaims) {
        ...
        const payload = {
            ...
            iat: Math.floor(Date.now() / 1000), // Criação
            exp: Math.floor(Date.now() / 1000) + this._duracaoToken, // Expiração
            nbf: Math.floor(Date.now() / 1000), // Não é válido antes
    ...
    validarToken(stringToken) {
        ...
        try {
            ...
            if (err instanceof jwt.TokenExpiredError) {
                ...
                console.error("Erro geral ao validar token:", err);
            }
            return false;
        }
    }
